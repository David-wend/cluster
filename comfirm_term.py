...
class term:
    ...
    def printInfo(self):
        temp_arr = [self.name, str(self.id), self.flag, str(self.create_time), str(self.high_time),
                    str(self.close_time),
                    str(self.std), str(self.mean), str(self.count), str(self.freq), str(self.time_freq),
                    str(self.continued_days),
                    str(self.exist_days)]
    ...
    def statistics(self, rows):
        ...
        temp_dataframe = DataFrame(np.zeros(delta.days + 1),
                                   index=pd.date_range(self.create_time, rows[len(rows) - 1, 0]))
        for row in rows:
            if self.id in row[1]:
                try:
